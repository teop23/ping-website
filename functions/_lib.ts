...
export const httpCardStore = ({
  ...
  return {
    async get(key) {
      ...
      const messageHeader = response.headers.get('x-ping-message');
      return {
        body,
        traits: traitsHeader ? decodeURIComponent(traitsHeader) : '',
    ...
    async put(key, body, traits, message) {
      ...
      if (message) putHeaders['X-Ping-Message'] = encodeURIComponent(message);
      const response = await fetchFn(url(`/cards/${encodeURIComponent(key)}`), {
        method: 'PUT',
        headers: putHeaders,
        body,
 ...
 */
export const CARD_RENDER_VERSION = 10;
